...
struct region : public location, public std::enable_shared_from_this<region>
{
...
struct state : public location, public std::enable_shared_from_this<state>
{
...
//-----------------------------------------------------------------------------
inline names_t state::initial_states() const
{
    names_t ists;
    ...
    for (const auto &[k, r] : _regions)
    {
        names_t rists(r->initial_states());
        ists.insert(rists.begin(), rists.end());
    }
    return ists;
}

inline names_t region::initial_states() const
{
    names_t ists;
    for (const auto &[k, s] : _substates)
    {
        if (s->_initial) {
            names_t rists(s->initial_states());
            ists.insert(rists.begin(), rists.end());
        }
    }
    return ists;
...
inline names_t state_machine::initial_states() const
{
    names_t ists;
    for (const auto &[k, r] : _regions)
    {
